...
A: Use `bigBlind` as the source of truth. FR-2 defines bands by big blind dollar ranges, and `bigBlind` is already a numeric value that maps directly. `tableStakes` is a display string stored in the ledger for context but does not drive band selection.
